...
-----END DSA PRIVATE KEY-----
  ...
  context "" do
    ...
    it "::set_extensions(certificate, request) は OpenSSL::X509::Extension オブジェクトの配列を返すこと" do
      ...
      ZZZ::CA::Utils.should_receive(:get_oid_from_extension)
                    .with(extension1)
                    .and_return('subjectAltName')
      ZZZ::CA::Utils.should_receive(:get_oid_from_extension)
                    .with(extension2)
                    .and_return('subjectAltName')
      ZZZ::CA::Utils.should_receive(:get_value_from_extension)
                    .with(extension1)
                    .and_return('DNS:foo.example.com')
      ZZZ::CA::Utils.should_receive(:get_value_from_extension)
                    .with(extension2)
                    .and_return('DNS:bar.example.com')
